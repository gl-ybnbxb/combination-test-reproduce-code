...
####======================================================================================================================================
runJTK_combination <- function(indata,JTKtime,minper=20,maxper=28, releaseNote=TRUE, para = FALSE, ncores = 1,
                               method='Cauchy',...)
{
  ...
  jtkx <- function(z, ampci=FALSE, conf=0.8) {      ###v3.1 'ampci=TRUE' for calculating amplitude confidence
    ...
    ####################
    ## UPDATED
    pvals_unlist = unlist(pvals)
    for (i in 1:length(pvals_unlist)){ #fix underflow and 1
      if (pvals_unlist[i] == 1){
        pvals_unlist[i] = 1-1/216
      }
    }
    
    JTK.combinedP <- transformation_global_test(pvals_unlist, method=method) # Combination test combination
    # JTK.combinedP <- ACAT(pvals_unlist)    
    ## UPDATED
    ####################
